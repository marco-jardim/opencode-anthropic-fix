...
| D12  | **The OIDC federation grant is not implemented.** No JWT-bearer request is ever emitted.   | Deliberate scope decision. Federation is a distinct authentication surface for service accounts and CI, not an enhancement of the interactive flow, and this plugin's users authenticate interactively. Implementing an unused grant would add a maintained code path, a second credential lifecycle, and a set of constants with no consumer — cost with no delivered value. §2.4 and §3's federation constants are recorded here so the grant can be added correctly later; they are absent from `lib/oauth-constants.mjs` by design, enforced by a test that fails if any `OIDC_*` export appears. Consequence: OIDC federation has no drift row in the parity plan — the row was removed on 2026-09-23 when the wave was cut as out of scope — and its closure is not claimed.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | permanent — scope              |
| D14  | `CLAUDE_CODE_CUSTOM_OAUTH_URL` is **validated, and an approved value is refused**          | §5.1. §13.8 (byte 4657393) attests that the variable is checked against a three-entry allowlist and that an unapproved value **throws** with a byte-exact message. It does not attest what an **approved** value is then used for. There are three options and only one is defensible. Redirecting the flow to the approved value invents a behaviour the evidence does not record. Accepting it and quietly continuing is worse than either alternative: an operator who points this plugin at a FedRAMP endpoint gets no error, and their authorization code and refresh token still go to `platform.claude.com` — a misroute dressed as a feature. So `authorize()` enforces the attested throw for an unapproved value, and refuses an approved one with an explicit "cannot honour it" error naming this divergence. **Consequence: drift row 7 is NOT fully closed.** The attested half — the rejection — is implemented; the use site needs a new offset. **Scope of the guard:** it runs in uthorize() only. efreshToken(), xchange() and evoke() never consult the environment — they post to the constant token URL — so an environment that sets this variable and only ever refreshes an already-stored token is unaffected and unwarned. That is deliberate: throwing on the refresh path would kill live sessions over a configuration the refresh does not use. No allowlist entry equals a default host, so no previously-working configuration becomes a hard failure. | until the use site is attested |
...
| D15b | `CLAUDE_LOCAL_OAUTH_*_BASE` have **no production caller**                                  | §5. `resolveAppsBase` / `resolveConsoleBase` transcribe the attested variables and defaults, are tested, and are guarded by D15 — but nothing in `lib/oauth.mjs` calls them, because §13.8 records the variables and their defaults and **not** which requests they redirect. Exported helpers with no caller are not a feature. **Consequence: drift row 8 is NOT closed.** It is recorded here as OPEN rather than counted, because claiming a closed row for code with no runtime effect is the kind of bookkeeping this document exists to prevent.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | open — needs a use site        |
| D17  | Refresh **timing**: the attested 30 s skew is a floor, and two larger leads sit above it   | §3. §13.8 attests xpiry skew (seconds): 30. isTokenExpired now guarantees a token is never treated as valid within 30 s of expiry — that half of drift row 11 is genuinely closed and a test pins the boundary. What is **not** closed, and is recorded as open rather than filed under a closed row: (a) the pre-existing **300 s foreground buffer**, so this plugin refreshes on demand far earlier than a client applying only the 30 s skew, and (b) the pre-existing **idle-refresh path**, which emits a refresh with no following inference request at all. Math.max(leadMs, 30 s) leaves both leads exactly as they were — the floor never binds under them — so neither is observably different from before this work, and a capture can still tell the two clients apart on refresh timing. The reason the default was not changed is a product choice about this codebase, not evidence about the genuine client: this is a streaming proxy, and a 30 s lead can expire a token during a long response it is already relaying. That rationale is **unverified against any measurement**. See open signals O9 and O10.                                                                                                                                                                                                                                                                                                                                                       | permanent — product choice     |
...
| O7  | Multi-process token rotation: a refresh is written to disk through a 1 s debounce, so a second process that acquires the refresh lock inside that window can still POST the superseded token. Pre-existing, unchanged by this programme, and unrelated to wire shape — recorded because a global review looked for it and it is real. Env-sourced accounts are **not** exposed to the related object-identity hazard: `syncActiveIndexFromDisk` re-appends the _same_ account objects rather than rebuilding them, so an in-flight refresh cannot land on an orphan. |
| O8  | `hasRequiredScopes` is exported and has **no caller anywhere in the repository**. Its predicate is unchanged by this programme — it checked the same five base scopes before and after being routed through `missingBaseScopes` — so no stored token's validity changed. `missingBaseScopes` itself has exactly two callers: this predicate, and the headless boot-time check in `lib/oauth-env.mjs`, which only warns. Neither gates a stored token. It is listed here because a dead exported predicate reads like a live gate to the next maintainer.             |
| O9  | **Foreground refresh lead.** This plugin refreshes on demand at T−300 s (`FOREGROUND_REFRESH_EXPIRY_BUFFER_MS`); a client applying only the attested 30 s skew would refresh at T−30 s. Pre-existing, unchanged, wire-visible as timing. See D17.                                                                                                                                                                                                                                                                                                                    |
| O10 | **Idle-refresh path.** A background pass refreshes tokens inside `idle_refresh.window_minutes`, emitting a token request with **no following inference request**. §13.8 records nothing about a background refresher in the genuine client, in either direction. Pre-existing, unchanged, wire-visible as both timing and request count. See D17.                                                                                                                                                                                                                    |
