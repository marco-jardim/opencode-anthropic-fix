...
| D17  | Refresh **timing** diverges: the attested 30 s skew is a floor, not the lead               | §3. §13.8 attests `expiry skew (seconds): 30`, and `isTokenExpired` now guarantees a token is never treated as valid within 30 s of expiry. But this plugin is a proxy that must not let a token die mid-stream, so its foreground path already refreshed at T−300 s and its idle path refreshes earlier still; `Math.max(leadMs, 30 s)` keeps those tuned leads. A packet capture therefore shows this client refreshing earlier than a genuine on-demand client, and the idle path emits a refresh with no following inference request at all. That is a real, wire-visible divergence and it is recorded here rather than hidden behind a closed drift row: **row 11 is closed only in the sense that the 30 s floor now exists everywhere.** Flipping the default lead to 30 s would trade a timing fingerprint for mid-request token death, which is the wrong trade for a proxy.                                                                           | permanent — product choice     |
