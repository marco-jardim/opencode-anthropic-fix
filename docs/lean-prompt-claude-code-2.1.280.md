...
> shorter prompt. See [§3](#3-unknown-the-prompt-text-delta).
...
>   at all; the predicate's later branches decide instead, and for the models
>   they name, those branches select the full prompt (see §1).
>
...
("do not send the full prompt"). The later branches only run when that lookup
yields `undefined`. The analysis document does not characterise `Ide()` or
`al()`, so this document does not guess what they test. Note that
...
§6). A genuine 2.1.280 client sends a **different** system prompt to a model
that carries `lean_prompt` than the one this plugin's mimicry otherwise assumes.

That difference is currently **uncharacterised, so it cannot be mimicked**.
...
**Evidence.** The analysis document in the wire-compat repository:
