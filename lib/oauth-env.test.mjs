...
describe("resolveHeadlessLogin (T-3 headless login)", () => {
  ...
  it("T-3.2c reports missing scopes in the five-scope base order", () => {
    ...
    ).toThrow("Missing: user:inference");
    expect(() =>
      resolveHeadlessLogin({
        CLAUDE_CODE_OAUTH_REFRESH_TOKEN: "rt-abc",
      ...
      "CLAUDE_CODE_OAUTH_SCOPES does not declare every scope this plugin requires. Missing: user:inference, user:sessions:claude_code, user:mcp_servers, user:file_upload",
    );
  });
