...
export function redactString(str) {
  try {
    return safeString(str)
      .replace(/Bearer(\s+|%20)([A-Za-z0-9._-]+)/gi, (_match, separator, credential) => {
        return `Bearer${separator}${hashMask(credential)}`;
      })
      .replace(/sk-ant-[A-Za-z0-9._-]+/gi, (token) => hashMask(token))
      .replace(/\boat01[A-Za-z0-9._-]{8,}\b/gi, (token) => hashMask(token));
  } catch {
    return "[redaction failed]";
  }
}
