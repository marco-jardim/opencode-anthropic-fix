...
describe("DEFAULT_CONFIG", () => {
  ...
  it("has expected default max wait", () => {
    expect(DEFAULT_CONFIG.max_rate_limit_wait_seconds).toBe(300);
  });
...
describe("loadConfig", () => {
  ...
  it("merges valid strategy from config file", () => {
    existsSync.mockReturnValue(true);
    readFileSync.mockReturnValue(
      JSON.stringify({ account_selection_strategy: "sticky" }),
    );
    const config = loadConfig();
  ...
  it("ignores invalid strategy", () => {
    existsSync.mockReturnValue(true);
    readFileSync.mockReturnValue(
      JSON.stringify({ account_selection_strategy: "invalid" }),
    );
    const config = loadConfig();
    expect(config.account_selection_strategy).toBe("hybrid");
  });

  it("clamps max_rate_limit_wait_seconds to valid range", () => {
    existsSync.mockReturnValue(true);
    readFileSync.mockReturnValue(
      JSON.stringify({ max_rate_limit_wait_seconds: 99999 }),
    );
    const config = loadConfig();
    expect(config.max_rate_limit_wait_seconds).toBe(3600);
  });

  it("clamps negative max_rate_limit_wait_seconds to 0", () => {
    existsSync.mockReturnValue(true);
    readFileSync.mockReturnValue(
      JSON.stringify({ max_rate_limit_wait_seconds: -10 }),
    );
    const config = loadConfig();
    expect(config.max_rate_limit_wait_seconds).toBe(0);
  });
