...
describe("getConfigDir", () => {
  ...
  it("uses APPDATA on Windows", () => {
    ...
    expect(getConfigDir()).toBe("C:\\Users\\test\\AppData\\Roaming\\opencode");
  ...
  it("uses XDG_CONFIG_HOME on non-Windows platforms", () => {
    ...
    expect(getConfigDir()).toBe("\\tmp\\custom-config\\opencode");
...
describe("loadConfig", () => {
  ...
  beforeEach(() => {
    ...
    delete process.env.OPENCODE_ANTHROPIC_LENGTH_ANCHORS;
  });
  ...
  it("validates and merges every supported nested config section", () => {
    ...
    readFileSync.mockReturnValue(
      JSON.stringify({
        ...
        anti_verbosity: { enabled: false, length_anchors: false },
    ...
    expect(config.anti_verbosity).toEqual({ enabled: false, length_anchors: false });
  ...
  it("applies false-form environment overrides for feature switches", () => {
    ...
    process.env.OPENCODE_ANTHROPIC_LENGTH_ANCHORS = "false";
