...
function calculateHybridScore(account, maxTokens) {
  ...
  // QA fix M17+: lastUsed=0 means never-used; treat as max freshness (3600s cap)
  // so new accounts aren't perpetually ranked below recently-active ones.
  const secondsSinceUsed = account.lastUsed > 0 ? (Date.now() - account.lastUsed) / 1000 : 3600;
...
export function selectAccount(candidates, strategy, currentIndex, healthTracker, tokenTracker, cursor) {
  ...
  switch (strategy) {
    ...
    case "single": {
      // Single-account mode: use exactly one account and never auto-rotate.
      // Stay on the current account while it is available; if it is
      // rate-limited or otherwise unavailable, STOP (return null) so the
      // caller alerts the user instead of silently switching accounts.
      // Auto-resumes once the account becomes available again (e.g. the
      // rate-limit window expires). See issue #20.
      if (currentIndex !== null) {
        const current = available.find((acc) => acc.index === currentIndex);
        return current ? { index: current.index, cursor } : null;
      }
      // No account selected yet — pick the first available as the single account.
      const next = available[cursor % available.length];
      return next ? { index: next.index, cursor: cursor + 1 } : null;
    }
