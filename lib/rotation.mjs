...
export class HealthScoreTracker {
  ...
  getScore(accountIndex) {
    ...
    const hoursSinceUpdate =
      (Date.now() - state.lastUpdated) / (1000 * 60 * 60);
    const recoveredPoints = Math.floor(
      hoursSinceUpdate * this.#config.recovery_rate_per_hour,
    );
  ...
  recordSuccess(accountIndex) {
    ...
    this.#scores.set(accountIndex, {
      score: Math.min(
        ...
        current + this.#config.success_reward,
      ),
...
export class TokenBucketTracker {
  ...
  getTokens(accountIndex) {
    ...
    const minutesSinceUpdate =
      (Date.now() - state.lastUpdated) / (1000 * 60);
    const recoveredTokens =
      minutesSinceUpdate * this.#config.regeneration_rate_per_minute;
...
export function selectAccount(
  ...
  const available = candidates.filter(
    (acc) => acc.enabled && !acc.isRateLimited,
  );
  ...
  switch (strategy) {
    case "sticky": {
      // Use current if available and healthy
      if (currentIndex !== null) {
        const current = available.find((acc) => acc.index === currentIndex);
        if (current && healthTracker.isUsable(current.index)) {
      ...
      return next
        ? { index: next.index, cursor: cursor + 1 }
        : null;
    }
    ...
    case "hybrid": {
      const scoredCandidates = available
        .filter(
          (acc) =>
            healthTracker.isUsable(acc.index) &&
    ...
    default:
      return available[0]
        ? { index: available[0].index, cursor }
        : null;
  }
}
