...
function validateStats(raw, now) {
  ...
  return {
    ...
    lastReset:
      typeof s.lastReset === "number" && Number.isFinite(s.lastReset)
        ? s.lastReset
        : now,
  };
}
...
function validateAccount(raw, now) {
  ...
  const addedAt =
    typeof acc.addedAt === "number" && Number.isFinite(acc.addedAt)
      ? acc.addedAt
      : now;
  ...
  const id =
    typeof acc.id === "string" && acc.id
      ? acc.id
      : `${addedAt}:${acc.refreshToken.slice(0, 12)}`;
  ...
  return {
    ...
    expires:
      typeof acc.expires === "number" && Number.isFinite(acc.expires)
        ? acc.expires
        : undefined,
    addedAt,
    lastUsed:
      typeof acc.lastUsed === "number" && Number.isFinite(acc.lastUsed)
        ? acc.lastUsed
        : 0,
    ...
    rateLimitResetTimes:
      ...
      !Array.isArray(acc.rateLimitResetTimes)
        ...
        : {},
    consecutiveFailures:
      typeof acc.consecutiveFailures === "number"
        ? Math.max(0, Math.floor(acc.consecutiveFailures))
        : 0,
    lastFailureTime:
      typeof acc.lastFailureTime === "number" ? acc.lastFailureTime : null,
    lastSwitchReason:
      typeof acc.lastSwitchReason === "string"
        ? acc.lastSwitchReason
        : undefined,
