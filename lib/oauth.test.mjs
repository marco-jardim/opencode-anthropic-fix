...
describe("oauth headers", () => {
  ...
  it("sends SDK native-fetch fingerprint by default on token exchange", async () => {
    ...
    expect(init.headers["User-Agent"]).toBe("anthropic-sdk-typescript/0.94.0 userOAuthProvider");
    expect(init.headers["anthropic-beta"]).toBe("oauth-2025-04-20");
    expect("Accept" in init.headers).toBe(false);
  });

  it("sends SDK native-fetch fingerprint by default on token refresh", async () => {
    mockFetch.mockResolvedValueOnce({
      ok: true,
  ...
  it("flag OFF (default) keeps axios fingerprint on exchange", async () => {
  ...
  it("flag ON sends SDK native-fetch fingerprint on exchange", async () => {
    mockFetch.mockResolvedValueOnce({ ok: false, status: 400, text: async () => "" });
  ...
  it("flag OFF (default) keeps axios fingerprint on refresh", async () => {
    mockFetch.mockResolvedValueOnce({
      ok: true,
      json: async () => ({ access_token: "a", refresh_token: "r", expires_in: 3600 }),
    });
  ...
  it("flag ON sends SDK native-fetch fingerprint on refresh", async () => {
    mockFetch.mockResolvedValueOnce({
      ok: true,
      json: async () => ({ access_token: "a", refresh_token: "r", expires_in: 3600 }),
    });

    await refreshToken("refresh-token", { sdkTokenUserAgent: true });
