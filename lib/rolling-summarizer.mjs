...
export function formatTemplate(parsed, maxChars) {
  ...
  while (out.length > maxChars && guard < 1000) {
    ...
    // Chop 10% (at least 8 chars) off the end of the longest section. Cut on
    // a grapheme boundary — never split a surrogate pair or combining
    // sequence — while still guaranteeing progress: the target is always
    // strictly below `longest.length` here, so truncateGraphemes always
    // shortens it (it only ever returns the input unchanged when the input
    // already fits the target).
    const chop = Math.max(8, Math.floor(longest.length * 0.1));
    const target = Math.max(EMPTY_SECTION.length, longest.length - chop);
    current = { ...current, [longestName]: truncateGraphemes(longest, target) };
    out = render(current);
  }

  // Final hard cap: if we somehow still exceed, cut the envelope on a
  // grapheme boundary instead of slicing UTF-16 units blindly.  Still
  // deterministic (input-driven only).
  if (out.length > maxChars) {
    const closing = "\n</session-summary>";
    if (maxChars <= closing.length) {
      // The budget doesn't even cover the closing tag by itself. Match the
      // pre-existing fallback shape — a bare, well-formed closing tag —
      // rather than emitting an envelope with no closing tag at all.
      out = closing;
    } else {
      out = truncateGraphemes(out, maxChars, { suffix: closing });
    }
  }
  return out;
}
