...
import { WIRE_PROFILE } from "./mimicry/wire-compat.mjs";
...
describe("buildDiagnosticBundle", () => {
  ...
  it("reports the emulated wire-compat profile and the loaded package version", async () => {
    const bundle = await buildDiagnosticBundle();

    expect(bundle.meta.mimicryBaseline).toBe(WIRE_PROFILE.cliVersion);
    expect(bundle.meta.mimicryProfileId).toBe(WIRE_PROFILE.id);
    // Reads through lib/mimicry/wire-compat.mjs's own resolution of
    // node_modules, which the mocked node:fs above degrades to "unknown"
    // (its existsSync mock only recognizes debug-headers.log and
    // request-dumps paths) -- covered against the real, unmocked filesystem
    // by lib/mimicry/wire-compat.test.mjs.
    expect(typeof bundle.meta.wireCompatPackageVersion).toBe("string");
    expect(bundle.meta.wireCompatPackageVersion.length).toBeGreaterThan(0);
  });
