...
describe("TokenBucketTracker", () => {
  ...
  it("consume reduces tokens", () => {
    tracker.consume(0);
    expect(tracker.getTokens(0)).toBeCloseTo(49, 1);
  });
  ...
  it("refund adds tokens back", () => {
    ...
    tracker.consume(0);
    tracker.refund(0);
    expect(tracker.getTokens(0)).toBeCloseTo(49, 0);
  });
...
describe("selectAccount", () => {
  ...
  describe("hybrid strategy", () => {
    it("selects the account with highest composite score", () => {
      const candidates = [
        ...
        makeCandidate({ index: 1, lastUsed: Date.now() - 120_000 }), // 2 min ago = more fresh
      ];
      // Account 1 should have higher freshness score (120s * 0.1 = 12pt bonus)
  ...
  // Single strategy (issue #20)
  describe("single strategy", () => {
    it("selects first available account when no current index", () => {
      const candidates = [makeCandidate({ index: 0 }), makeCandidate({ index: 1 })];
      const result = selectAccount(candidates, "single", null, healthTracker, tokenTracker, 0);
      expect(result?.index).toBe(0);
    });

    it("stays on the current account when it is available", () => {
      const candidates = [makeCandidate({ index: 0 }), makeCandidate({ index: 1 })];
      const result = selectAccount(candidates, "single", 0, healthTracker, tokenTracker, 0);
      expect(result?.index).toBe(0);
      expect(result?.cursor).toBe(0);
    });

    it("returns null when current is rate-limited (does NOT switch)", () => {
      const candidates = [makeCandidate({ index: 0, isRateLimited: true }), makeCandidate({ index: 1 })];
      const result = selectAccount(candidates, "single", 0, healthTracker, tokenTracker, 0);
      // Unlike sticky, single must NOT fall through to account 1.
      expect(result).toBeNull();
    });

    it("returns null when the current account is gone (disabled/removed)", () => {
      const candidates = [makeCandidate({ index: 1 })];
      const result = selectAccount(candidates, "single", 0, healthTracker, tokenTracker, 0);
      expect(result).toBeNull();
    });

    it("ignores health score and stays on current", () => {
      for (let i = 0; i < 5; i++) {
        healthTracker.recordFailure(0);
      }
      const candidates = [makeCandidate({ index: 0 }), makeCandidate({ index: 1 })];
    ...
    it("auto-resumes the current account once its rate-limit clears", () => {
      const limited = [makeCandidate({ index: 0, isRateLimited: true }), makeCandidate({ index: 1 })];
      expect(selectAccount(limited, "single", 0, healthTracker, tokenTracker, 0)).toBeNull();
      const cleared = [makeCandidate({ index: 0 }), makeCandidate({ index: 1 })];
      const result = selectAccount(cleared, "single", 0, healthTracker, tokenTracker, 0);
      expect(result?.index).toBe(0);
    });
  });
