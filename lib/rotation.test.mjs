...
describe("selectAccount", () => {
  ...
  describe("sticky strategy", () => {
    it("returns current account if available and healthy", () => {
      const candidates = [
    ...
    it("switches when current is unhealthy", () => {
      ...
      // Sticky fallback uses cursor-based selection from available list.
      // Account 0 is still available (not rate-limited), so cursor 0 picks it.
      // The key behavior is that it does NOT return via the "stay on current" path
      // (which would keep cursor unchanged). Instead it advances the cursor.
      expect(result?.cursor).toBe(1); // Cursor advanced = didn't use sticky path
    });

    it("prefers healthy account when current is unhealthy and cursor points to healthy one", () => {
      // Make account 0 unhealthy
      for (let i = 0; i < 5; i++) {
        healthTracker.recordFailure(0);
      }
      ...
      // cursor=1 should pick account 1 (the healthy one)
      const result = selectAccount(
