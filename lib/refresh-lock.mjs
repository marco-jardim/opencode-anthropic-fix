    ...
    try {
      ...
      try {
        ...
        if (Date.now() - stat.mtimeMs > staleMs) {
          // QA fix L-toctou: re-stat after potential concurrent lock renewal
          // to reduce the TOCTOU window. If mtime changed, skip the unlink.
          try {
            const reStat = await fs.stat(lockPath);
            if (Date.now() - reStat.mtimeMs > staleMs) {
              await fs.unlink(lockPath);
            }
          } catch {
            // File vanished between stat calls — another process released it
          }
          continue;
        }
      } catch {
