...
describe("loadAccounts", () => {
  ...
  it("returns best-effort data for newer version (forward compat)", async () => {
    // QA fix L-warnSpy: mock console.warn to prevent test output contamination
    const warnSpy = vi.spyOn(console, "warn").mockImplementation(() => {});
    fs.readFile.mockResolvedValue(JSON.stringify({ version: 99, accounts: [] }));
    const result = await loadAccounts();
    // H12 fix: no longer returns null; attempts best-effort load with a warning
    expect(result).not.toBeNull();
    expect(result.accounts).toEqual([]);
    expect(result.version).toBe(1); // normalized to current version
...
describe("saveAccounts", () => {
  ...
  it("keeps persisted JSON consistent when concurrent saves commit out of order", async () => {
    randomBytes
      .mockReturnValueOnce({ toString: () => "writer-one" })
      .mockReturnValueOnce({ toString: () => "writer-two" });
    ...
    fs.writeFile.mockImplementation(async (path, content) => {
      if (path.includes("writer-one")) {
        markFirstWriteStarted();
        await firstWriteBlocked;
      }
      files.set(path, content);
    });
    fs.rename.mockImplementation(async (tempPath, storagePath) => {
      const content = files.get(tempPath);
      if (content === undefined) throw new Error("temp file missing");
      files.set(storagePath, content);
      files.delete(tempPath);
    });

    const firstStorage = {
      version: 1,
      accounts: [{ refreshToken: "first-writer" }],
      activeIndex: 0,
    };
    const secondStorage = {
      version: 1,
      accounts: [{ refreshToken: "second-writer" }],
      activeIndex: 0,
    };
    const emptyDisk = { version: 1, accounts: [], activeIndex: 0 };

    const firstSave = saveAccounts(firstStorage, emptyDisk);
    await firstWriteStarted;
    await saveAccounts(secondStorage, emptyDisk);
    releaseFirstWrite();
    await firstSave;

    const storagePath = fs.rename.mock.calls.at(-1)[1];
    const persisted = JSON.parse(files.get(storagePath));
    expect(persisted).toEqual(firstStorage);
    expect([...files.keys()].filter((path) => path.endsWith(".tmp"))).toEqual([]);
  });

  itWindows("propagates a Windows ACL-denied write and removes the temp file", async () => {
    const accessDenied = Object.assign(new Error("access denied"), { code: "EACCES" });
    fs.writeFile.mockRejectedValue(accessDenied);
    fs.unlink.mockResolvedValue(undefined);

    const storage = { version: 1, accounts: [], activeIndex: 0 };
    await expect(saveAccounts(storage, storage)).rejects.toBe(accessDenied);

    const tempPath = fs.writeFile.mock.calls[0][0];
  ...
  it("cleans up the temp file when atomic rename fails", async () => {
    ...
    const storage = { version: 1, accounts: [], activeIndex: 0 };
    await expect(saveAccounts(storage, storage)).rejects.toBe(renameError);

    const tempPath = fs.writeFile.mock.calls[0][0];
    expect(fs.rename).toHaveBeenCalledWith(tempPath, expect.stringMatching(/anthropic-accounts\.json$/));
    expect(fs.unlink).toHaveBeenCalledWith(tempPath);
  });
