...
describe("request-headers constants", () => {
  it("FALLBACK_CLAUDE_CLI_VERSION is 2.1.133", () => {
    expect(FALLBACK_CLAUDE_CLI_VERSION).toBe("2.1.133");
  });
...
describe("getSdkVersion", () => {
  it("returns known SDK version for CLI version 2.1.133", () => {
    // 2.1.133 is in the VERSION_TO_SDK_MAP
    const v = getSdkVersion("2.1.133");
...
describe("buildExtendedUserAgent", () => {
  ...
  it("returns default cli format without env vars", () => {
    ...
    delete process.env.CLAUDE_AGENT_SDK_VERSION;
    delete process.env.CLAUDE_AGENT_SDK_CLIENT_APP;
    const ua = buildExtendedUserAgent("2.1.133");
    expect(ua).toBe("claude-cli/2.1.133 (external, cli)");
  });

  it("uses CLAUDE_CODE_ENTRYPOINT when set", () => {
    process.env.CLAUDE_CODE_ENTRYPOINT = "vscode";
    const ua = buildExtendedUserAgent("2.1.133");
    expect(ua).toBe("claude-cli/2.1.133 (external, vscode)");
  });
  ...
  it("appends agent-sdk suffix when CLAUDE_AGENT_SDK_VERSION is set", () => {
    ...
    expect(ua).toMatch(/^claude-cli\/2\.1\.133/);
  });
