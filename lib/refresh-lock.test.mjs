...
describe("refresh-lock", () => {
  ...
  it("does not release when inode changed even if owner matches", async () => {
    ...
    // Create the replacement while the original still exists, guaranteeing distinct inodes.
    const replacementPath = `${first.lockPath}.replacement`;
    await fs.writeFile(replacementPath, JSON.stringify({ owner: first.owner, createdAt: Date.now() }), {
      encoding: "utf-8",
      mode: 0o600,
    });
    const replacementStat = await fs.stat(replacementPath);
    expect(replacementStat.ino).not.toBe(first.lockInode);

    await fs.unlink(first.lockPath);
    await fs.rename(replacementPath, first.lockPath);
