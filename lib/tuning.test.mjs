...
describe("tuning constants", () => {
  ...
  it("matches the inline token refresh timeout and foreground expiry buffer", () => {
    // `signal` must stay the FIRST option so the timeout remains pinned to the
    // tuning constant, but the options object is allowed to carry further keys:
    // headless env accounts add `clientId` (OAuth contract §5.2, divergence D10).
    ...
    // Stronger pin: the value now comes from the tuning module by import,
    // rather than a duplicated literal that the regex had to police.
    expect(indexSource).toMatch(/isTokenExpired\(\s*account\.expires\s*,\s*FOREGROUND_REFRESH_EXPIRY_BUFFER_MS\s*\)/);
    expect(indexSource).toMatch(
      /import\s*\{[^}]*\bFOREGROUND_REFRESH_EXPIRY_BUFFER_MS\b[^}]*\}\s*from\s*"\.\/lib\/tuning\.mjs"/,
    );
    expect(indexSource).not.toMatch(
      /(?:\.expires|\.expiresAt|\bexpires\b)\s*(?:<=?|>=?)\s*[^\n;{}]*(?:300_000|300000)\b/,
    );
  });
