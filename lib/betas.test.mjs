...
import { BETA_REGISTRY_2_1_280 } from "./mimicry/wire-compat.mjs";
 ...
 *
 * These tests read the package's own 2.1.280 registry through the seam and
 ...
 */
const REGISTRY_HEADERS = new Set(Object.values(BETA_REGISTRY_2_1_280).map((entry) => entry.header));
...
/**
 * Betas the HOST tracks that the 2.1.280 registry has no entry for.
 *
...
describe("host beta tables reconciled with the wire package registry", () => {
  it("keeps every shared experimental beta byte-equal to the upstream header", () => {
    ...
    expect(drifted, "EXPERIMENTAL_BETA_FLAGS has stale date stamps against BETA_REGISTRY_2_1_280").toEqual([]);
  });
  ...
  it("keeps every shortcut target byte-equal to the upstream header for its family", () => {
    ...
    expect(drifted, "BETA_SHORTCUTS has stale date stamps against BETA_REGISTRY_2_1_280").toEqual([]);
  });
});
