...
/** Cap on cooldowns derived from reset headers. Prevents lock-out from buggy timestamps. */
const MAX_COOLDOWN_FROM_RESET_MS = 300_000; // 5 minutes

/** Threshold below which a 429 retry-after is considered transient (burst throttle). */
export const TRANSIENT_RETRY_THRESHOLD_MS = 10_000; // 10 seconds

/**
 * Parse the retry-after-ms header from a response.
 * Returns milliseconds directly (no conversion needed).
 * @param {Response} response
 * @returns {number | null}
 */
export function parseRetryAfterMsHeader(response) {
...
/**
 * Parse the x-should-retry header from a response.
 * @param {Response} response
 * @returns {boolean | null} true = force retry, false = force no retry, null = not present
 */
export function parseShouldRetryHeader(response) {
...
/**
 * Parse the unified rate-limit reset header from a response (Claude Code 2.1.195+).
 * Reads `anthropic-ratelimit-unified-reset`, falling back to
 * `anthropic-ratelimit-unified-overage-reset` when the primary header is absent.
 * Both are UNIX epochs in SECONDS (parsed via `Number()`, mirroring upstream).
 * Returns milliseconds-until-reset, or null when the header is missing, not a
 * number, or already in the past.
 * @param {Response} response
 * @returns {number | null} Milliseconds until reset, or null
 */
export function parseUnifiedResetMsHeader(response) {
  const header =
    response.headers.get("anthropic-ratelimit-unified-reset") ??
    response.headers.get("anthropic-ratelimit-unified-overage-reset");
  if (!header) return null;
...
function bodyHasAccountError(body) {
  const { errorType, message } = extractErrorSignals(body);
  ...
  // QA fix: only match signals against error type and parsed message fields,
  // not the full JSON body text — common words like "billing", "permission" in
  // unrelated JSON fields (e.g. resource URLs) cause false account-specific
  // classification, triggering unnecessary account rotation.
  return (
...
export function calculateBackoffMs(reason, consecutiveFailures, retryAfterMs) {
  // Retry-After header takes precedence, but cap at MAX_COOLDOWN to prevent lock-out
