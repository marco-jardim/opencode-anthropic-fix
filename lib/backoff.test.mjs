...
describe("isAccountSpecificError", () => {
  ...
  it("returns true for 400 with rate limit language in JSON error message", () => {
    expect(
      isAccountSpecificError(
        400,
        JSON.stringify({
          error: { type: "invalid_request_error", message: "This request would exceed your account's rate limit" },
        }),
      ),
    ).toBe(true);
  });

  it("returns true for 400 with quota language in JSON error message", () => {
    expect(
      isAccountSpecificError(
        400,
        JSON.stringify({ error: { type: "invalid_request_error", message: "Your quota has been exhausted" } }),
      ),
    ).toBe(true);
  });

  it("returns true for 400 with credit balance language in JSON error message", () => {
    expect(
      isAccountSpecificError(
        400,
        JSON.stringify({
          error: { type: "invalid_request_error", message: "Your credit balance is too low to complete this request" },
        }),
      ),
    ).toBe(true);
  });

  it("returns true for 400 with billing language in JSON error message", () => {
    expect(
      isAccountSpecificError(
        400,
        JSON.stringify({ error: { type: "billing_error", message: "Billing issue on your account" } }),
      ),
    ).toBe(true);
  });

  it("returns true for 403 with permission language in JSON error message", () => {
    expect(
      isAccountSpecificError(
        403,
        JSON.stringify({
          error: { type: "permission_error", message: "You do not have permission to access this model" },
        }),
      ),
    ).toBe(true);
  });

  it("returns false for plain text body without JSON structure (no false positives)", () => {
    // Plain text bodies that happen to contain keywords should NOT match
    // — only structured error.type and error.message fields are checked
    expect(isAccountSpecificError(400, "This request would exceed your account's rate limit")).toBe(false);
    expect(isAccountSpecificError(400, "Billing issue on your account")).toBe(false);
    expect(isAccountSpecificError(403, "You do not have permission to access this model")).toBe(false);
  });
  ...
  it("is case-insensitive for JSON error message matching", () => {
    expect(
      isAccountSpecificError(
        400,
        JSON.stringify({ error: { type: "invalid_request_error", message: "RATE LIMIT exceeded" } }),
      ),
    ).toBe(true);
    expect(
      isAccountSpecificError(
        400,
        JSON.stringify({ error: { type: "invalid_request_error", message: "QUOTA Exhausted" } }),
      ),
    ).toBe(true);
  });
});
...
describe("parseUnifiedResetMsHeader", () => {
  it("returns null when no unified-reset header present", () => {
    const response = new Response(null, { headers: {} });
    expect(parseUnifiedResetMsHeader(response)).toBeNull();
  });

  it("returns ~ms-until-reset for a future epoch (seconds)", () => {
    const futureEpochSeconds = Math.floor(Date.now() / 1000) + 120;
    const response = new Response(null, {
      headers: { "anthropic-ratelimit-unified-reset": String(futureEpochSeconds) },
    });
    const result = parseUnifiedResetMsHeader(response);
    expect(result).toBeGreaterThan(110_000);
    expect(result).toBeLessThanOrEqual(120_000);
  });

  it("returns null for a non-numeric value (NaN)", () => {
    const response = new Response(null, {
  ...
  it("returns null for a past epoch (<= now)", () => {
    const pastEpochSeconds = Math.floor(Date.now() / 1000) - 60;
    const response = new Response(null, {
      headers: { "anthropic-ratelimit-unified-reset": String(pastEpochSeconds) },
    });
    expect(parseUnifiedResetMsHeader(response)).toBeNull();
  });

  it("falls back to overage-reset when unified-reset is absent", () => {
    const futureEpochSeconds = Math.floor(Date.now() / 1000) + 90;
    const response = new Response(null, {
      headers: { "anthropic-ratelimit-unified-overage-reset": String(futureEpochSeconds) },
    });
    const result = parseUnifiedResetMsHeader(response);
    expect(result).toBeGreaterThan(80_000);
    expect(result).toBeLessThanOrEqual(90_000);
  });

  it("prefers unified-reset over overage-reset when both present", () => {
    const resetEpoch = Math.floor(Date.now() / 1000) + 120;
    const overageEpoch = Math.floor(Date.now() / 1000) + 300;
    const response = new Response(null, {
      headers: {
        "anthropic-ratelimit-unified-reset": String(resetEpoch),
        "anthropic-ratelimit-unified-overage-reset": String(overageEpoch),
      },
    });
    const result = parseUnifiedResetMsHeader(response);
    expect(result).toBeGreaterThan(110_000);
