...
describe("readCCCredentialsFromKeychain", () => {
  ...
  it("reads single keychain service on macOS", () => {
    ...
    // dump-keychain uses execSync, find-generic-password uses execFileSync
    execSync.mockImplementation((cmd) => {
      if (cmd === "security dump-keychain") return KEYCHAIN_DUMP_SINGLE;
      throw new Error("unexpected execSync cmd");
    });
    execFileSync.mockImplementation((_bin, args) => {
      if (args && args.includes("find-generic-password")) return JSON.stringify(VALID_CRED);
...
describe("readCCCredentials", () => {
  ...
  it("combines keychain and file sources on macOS", () => {
    ...
    // Keychain: dump-keychain via execSync, find-generic-password via execFileSync
    execSync.mockImplementation((cmd) => {
      if (cmd === "security dump-keychain") return KEYCHAIN_DUMP_SINGLE;
      throw new Error("unexpected execSync cmd");
    });
  ...
  it("deduplicates by refreshToken, preferring keychain", () => {
    ...
    execFileSync.mockImplementation((_bin, args) => {
      if (args && args.includes("find-generic-password")) return JSON.stringify(keychainCred);
      throw new Error("unexpected execFileSync cmd");
    });
