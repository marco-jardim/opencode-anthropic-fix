...
import { EXPERIMENTAL_BETA_FLAGS, BETA_SHORTCUTS, buildExtendedUserAgent } from "../request-headers.mjs";
...
export function detectProvider(requestUrl) {
...
export function buildAnthropicBetaHeader(
  ...
  // 400/409/529 errors referencing the hint. Users must explicitly opt in via
  // token_economy.context_hint = true, matching the genuine flag's false default.
  ...
  if (isFirstPartyProvider && !CLAUDE_3_MODEL_RE.test(model) && te.context_hint === true && _isMainThread) {
