 ...
 *
 * It survives for two callers and no more:
 *  1. `buildRequestHeaders` below, i.e. the endpoints the package has no
 *     surface for (files, models, a gateway-prefixed route);
 *  2. the `computedBetaHeader` call in `index.mjs`, which no longer reaches the
 *     wire at all — it exists only so `transformRequestBody` can test for
 *     `task-budgets-2026-03-13`. Flagged there for Phase 4.1.
 *
