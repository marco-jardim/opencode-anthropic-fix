...
import { shouldEmitContextHintBody } from "./context-hint-threshold.mjs";
...
export function transformRequestBody(body, signature, runtime, betaHeader, config) {
  ...
  try {
    ...
    // CC v2.1.195 emits the context-hint beta independently, but only emits the
    // body field when clearing old tool results would save at least 20K tokens.
    // Adapter path only: the shared package owns the context_hint body field and
    // gates it on its own profile flag (`contextHintEnabled`), so injecting it
    // here would either duplicate or contradict that decision. CONDITIONAL,
    // never unconditional: the legacy path still injects it.
    if (
      signature.useAdapter !== true &&
      betaHeader &&
      betaHeader.includes("context-hint-2026-04-09") &&
      !parsed.context_hint &&
      shouldEmitContextHintBody(parsed.messages)
    ) {
