...
const WIRE_COMPAT_PACKAGE_NAME = "@tormentalabs/claude-code-wire-compat";
 ...
 *
 * The CAPABILITY predicates (`isHaikuModel`, `supportsStructuredOutputs`,
 * `supportsWebSearch`) join them for the same reason. `lib/mimicry/headers.mjs`
 * still declares its own local copies, but those now feed ONLY the frozen legacy
 * header forge; every adapter-path decision reads the package's, which are
 * ported from the genuine client's own capability table rather than guessed from
 * a regex. The one measured behavioural difference —
 * `supportsStructuredOutputs("claude-haiku-4-5")` is `true` upstream and was
 * `false` under the plugin's "haiku never supports it" heuristic — is a mimicry
 * fidelity FIX, and is pinned by the migration harness baseline.
 */
export {
  hasOneMillionContext,
  isAdaptiveThinkingModel,
...
/**
 * The genuine client's beta registry for the profile the plugin emulates,
 * re-exported so the host's own beta tables can be RECONCILED against it
 * without a second import point.
 *
 * `lib/betas.mjs` is host policy — what the plugin adds or suppresses on top of
 * what the package composes — and it names beta headers as literal strings. A
 * literal is exactly the thing that rots when upstream bumps a beta's date, and
 * nothing used to notice. `lib/betas.test.mjs` now walks this registry and fails
 * on a header that has drifted from it, so the rot is caught at test time rather
 * than on the wire.
 *
 * Shape: `{ KEY: { featureKey, header } }`, deep-frozen by the package. It is
 * pinned to 2.1.280 rather than derived from `WIRE_PROFILE` because the package
 * exports no profile->registry accessor; if the emulated baseline moves, this
 * binding and `WIRE_PROFILE` move together.
 */
export { BETA_REGISTRY_2_1_280 };
 ...
 * eligible under 2.1.280 — so calling it bare would gate the plugin's context
 ...
 *
 * THIS IS ALSO THE PROFILE THE REQUEST PATH ACTUALLY USES, not merely a
 * reporting baseline. `buildWireCompatibleRequest` and
 * `buildWireCompatibleCountTokensRequest` both pass this constant as the
 * explicit `profile` argument to `buildClaudeCodeRequest` /
 * `buildClaudeCodeCountTokensRequest` (see the package's own 0.6.0 CHANGELOG:
 * "the pin has to be passed on every call to each of the three entry points
 * that read the default seam -- pinning the builders alone is not enough").
 *
 * WHY AN EXPLICIT PASS, RATHER THAN LETTING THE CALL INHERIT THE PACKAGE'S
 * `DEFAULT_PROFILE`. The dependency is pinned to the `latest` dist-tag
 * (`docs/shared-package-provenance.md`) precisely so package releases --
 * fixes, new opt-in fields -- reach the plugin without a code change. Before
 * this constant was threaded through, an unnamed `profile` argument meant the
 * EMULATED CLIENT VERSION floated on that same tag: 0.6.0 moved
 * `DEFAULT_PROFILE` to 2.1.280 in a minor release and broke 48 plugin tests
 * that were still asserting 2.1.233 bytes. Passing `WIRE_PROFILE` explicitly on
 * every entry point decouples the two axes: `latest` keeps bringing library
 * fixes, and the emulated profile now moves only when this binding is
 * deliberately repointed at a different exported profile singleton -- a
 * one-line change reviewed like any other wire change, not a side effect of
 * `npm update`.
 *
 * @type {import('@tormentalabs/claude-code-wire-compat').ClaudeCodeProtocolProfile}
 */
export const WIRE_PROFILE = CLAUDE_CODE_2_1_280_PROFILE;

/**
 * Version of `@tormentalabs/claude-code-wire-compat` baked in at BUILD time by
 * `scripts/build.mjs`'s esbuild `define`, read from the package that was
 * actually installed in `node_modules` when the bundle was produced.
 *
 * WHY THIS EXISTS. `getWireCompatPackageVersion` below resolves the version at
 * RUNTIME through `import.meta.resolve`, which requires the package to still
 * be a separate, independently resolvable module -- true for an unbundled
 * import (tests, `node cli.mjs`, a host importing `index.mjs` directly) but
 * false for the bundles `scripts/build.mjs` produces: esbuild INLINES the
 * package's source into the single output file, so there is no
 * `node_modules/@tormentalabs/claude-code-wire-compat` for the bundle to
 * resolve at its own runtime, and the resolution silently degrades to
 * "unknown" -- defeating the diagnostic bundle's whole point of reporting what
 * the running process actually built against.
 *
 * `__WIRE_COMPAT_PACKAGE_VERSION__` is a bare, undeclared global identifier,
 * not an import and not `process.env`, because esbuild's `define` performs a
 * textual substitution of exactly that shape at bundle time. `typeof` never
 * throws on an undeclared identifier, so referencing it this way is exactly as
 * safe unbundled as it is bundled: unbundled, esbuild never touched this file,
 * the identifier stays undeclared, `typeof` reads `"undefined"`, and this
 * constant is `undefined` -- `getWireCompatPackageVersion` then falls through
 * to the runtime resolution exactly as it did before this constant existed.
 *
 * @type {string | undefined}
 */
/* global __WIRE_COMPAT_PACKAGE_VERSION__:readonly */
const BUNDLED_WIRE_COMPAT_PACKAGE_VERSION =
  typeof __WIRE_COMPAT_PACKAGE_VERSION__ === "string" ? __WIRE_COMPAT_PACKAGE_VERSION__ : undefined;

/**
 * The version of @tormentalabs/claude-code-wire-compat this process actually
 * runs against: the build-time injected version when bundled (see
 * `BUNDLED_WIRE_COMPAT_PACKAGE_VERSION` above), otherwise the version
 * resolved fresh from `node_modules` on every call rather than cached at
 * module load.
 *
 * WHY THE RUNTIME FALLBACK EXISTS. `package.json` pins the dependency to the
 * `latest` dist-tag (see docs/shared-package-provenance.md), so the version an
 * UNBUNDLED run actually runs against is whatever `package-lock.json` resolved
 * at install time -- it is not a literal anywhere in this source tree. A
 * diagnostic bundle that wants to say what wire the running process is
 * actually built against (as opposed to `WIRE_PROFILE.cliVersion`, the
 * BASELINE this file pins) has to read it off disk.
 *
 * Deliberately resolves through `node_modules`, not `package-lock.json`: the
 * lock says what a future `npm ci` WOULD install, `node_modules` says what
 * THIS process actually loaded, and the two can disagree after an `npm
 * install` that has not been re-run (see `scripts/check-wire-compat-drift.mjs`,
 * which checks exactly that disagreement).
 *
 * Resolution failure degrades to "unknown" rather than throwing -- a
 * diagnostic bundle must never fail to build over a missing package.json.
 *
 * Resolves via `import.meta.resolve`, not `createRequire(...).resolve`: the
 * package is ESM-only (its `exports` map declares an `import` condition and
 * no `require` one), so the CJS resolver fails it with "No 'exports' main
 * defined" even though the package is genuinely installed.
 *
 * @returns {string}
 */
export function getWireCompatPackageVersion() {
  if (BUNDLED_WIRE_COMPAT_PACKAGE_VERSION) return BUNDLED_WIRE_COMPAT_PACKAGE_VERSION;
  try {
    let directory = path.dirname(fileURLToPath(import.meta.resolve(WIRE_COMPAT_PACKAGE_NAME)));
    // Walk up from the package's resolved entry file (e.g. .../dist/index.js)
    // to the package.json alongside its root -- the entry file is not
    // guaranteed to be a direct sibling of it.
    for (let depth = 0; depth < 6; depth += 1) {
      const candidate = path.join(directory, "package.json");
      if (existsSync(candidate)) {
        const manifest = JSON.parse(readFileSync(candidate, "utf8"));
        if (manifest.name === WIRE_COMPAT_PACKAGE_NAME && typeof manifest.version === "string") {
...
export async function buildWireCompatibleRequest(body, transport) {
  ...
  let built;
  try {
    // `WIRE_PROFILE` is passed EXPLICITLY rather than omitted -- see its own
    // doc comment above for why an omitted argument here would let a future
    // package release move the emulated client profile out from under the
    // `latest` dist-tag.
    built = await buildClaudeCodeRequest(toClaudeCodeRequestInput(parsed, transport), WIRE_PROFILE);
  } catch (error) {
 ...
 * received from the host. Mirrors `buildWireCompatibleRequest`, including
 * passing `WIRE_PROFILE` as the explicit `profile` argument so the count turn
 * is pinned to the same emulated client profile as the main turn, rather than
 * inheriting whatever the installed package declares as its `DEFAULT_PROFILE`.
 *
 * @param {string | undefined} body
...
export async function buildWireCompatibleCountTokensRequest(body, transport) {
  ...
  let built;
  try {
    // See `buildWireCompatibleRequest`'s equivalent comment: explicit on
    // purpose, so the count-tokens surface cannot silently move to a different
    // profile than the main turn on a package bump.
    built = await buildClaudeCodeCountTokensRequest(toClaudeCodeCountTokensInput(parsed, transport), WIRE_PROFILE);
  } catch (error) {
    throw foldSafeDetailsIntoMessage(error);
  }
  return { ...built, headers: new Headers(built.headers) };
