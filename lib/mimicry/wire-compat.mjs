...
/**
 * Cache-breakpoint placement lives here because it is plugin-owned token-economy
 * policy, while the shared package owns wire-protocol identity. The package pins
 * cache control on its identity block, forbids it on billing, and leaves caller
 * blocks for the caller to decorate.
 *
 * @param {readonly import('@tormentalabs/claude-code-wire-compat').SystemInput[] | undefined} system
 * @param {Record<string, import('@tormentalabs/claude-code-wire-compat').JsonValue> | undefined} systemCacheControl
 * @returns {readonly import('@tormentalabs/claude-code-wire-compat').SystemInput[] | undefined}
 */
function applySystemCacheControl(system, systemCacheControl) {
  if (systemCacheControl === undefined) return system;

  const prototype =
    systemCacheControl === null || typeof systemCacheControl !== "object"
      ? undefined
      : Object.getPrototypeOf(systemCacheControl);
  if (prototype !== Object.prototype && prototype !== null) {
    throw new TypeError("Expected systemCacheControl to be a non-null plain object");
  }
  if (!system || system.length === 0) return system;

  return system.map((block, index) => {
    if (typeof block === "string") {
      return index === system.length - 1
        ? { type: "text", text: block, cache_control: { ...systemCacheControl } }
        : block;
    }
    return index === system.length - 1 ? { ...block, cache_control: { ...systemCacheControl } } : { ...block };
  });
}
 ...
 *   systemCacheControl?: Record<string, import('@tormentalabs/claude-code-wire-compat').JsonValue>,
...
function toClaudeCodeRequestInput(body, transport) {
  const system = applySystemCacheControl(normalizeSystem(body.system), transport.systemCacheControl);
