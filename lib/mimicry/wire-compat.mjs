...
/**
 * The header names the package composes itself, mirrored from `HEADER_NAMES`
 * (src/headers.ts). Exported so `wire-compat.test.mjs` can pin the mirror
 * against the package's real behaviour rather than trusting this copy.
 */
export const PACKAGE_CANONICAL_HEADER_NAMES = new Set([
  "anthropic-beta",
...
/** Mirror of the package's `FORBIDDEN_HEADER_NAMES` (src/headers.ts). */
export const PACKAGE_FORBIDDEN_HEADER_NAMES = new Set([
  "x-api-key",
  "cookie",
  "set-cookie",
  "forwarded",
...
/** Mirror of the package's `isForbiddenHeader` prefix rules. */
export const PACKAGE_FORBIDDEN_HEADER_PREFIXES = Object.freeze(["proxy-", "x-forwarded-"]);

/**
 * Apply the package's `dropConflicting` extra-header policy PLUGIN-SIDE.
 *
 * WHY THIS EXISTS. `extraHeaderPolicy` is not one of the sixteen keys
 * `ClaudeCodeCountTokensInput` picks, and `validateCountTokensInput` runs
 * `assertExactKeys`, so passing it is `INVALID_INPUT` rather than a no-op. The
 * count surface therefore always resolves extra headers under `strict`, where
 * the FIRST host header the package owns raises `DUPLICATE_HEADER` (canonical
 * name) or `FORBIDDEN_HEADER` (hop-by-hop / credential name) and nothing
 * reaches the wire. The plugin forwards a heterogeneous host header map — the
 * opencode SDK alone sends `content-type` and `accept` — so without this filter
 * every count turn would throw and fall back to nothing at all.
 *
 * The main turn gets the same outcome for free through
 * `extraHeaderPolicy: "dropConflicting"` on the transport, which the count pick
 * discards; this reproduces it. Only OWNERSHIP conflicts are dropped, exactly
 * as the package does: header syntax is still validated package-side and a
 ...
 */
function dropConflictingExtraHeaders(extraHeaders) {
  if (extraHeaders === undefined) return undefined;
  const kept = extraHeaders.filter(([name]) => {
    const normalized = String(name).toLowerCase();
    if (PACKAGE_CANONICAL_HEADER_NAMES.has(normalized)) return false;
    if (PACKAGE_FORBIDDEN_HEADER_NAMES.has(normalized)) return false;
    return !PACKAGE_FORBIDDEN_HEADER_PREFIXES.some((prefix) => normalized.startsWith(prefix));
  });
...
export function toClaudeCodeCountTokensInput(body, transport) {
  ...
  const extraHeaders = dropConflictingExtraHeaders(input.extraHeaders);
  if (extraHeaders === undefined) delete input.extraHeaders;
  else input.extraHeaders = extraHeaders;
