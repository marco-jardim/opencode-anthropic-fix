...
/**
 * The version of @tormentalabs/claude-code-wire-compat actually resolved in
 * `node_modules`, read fresh on every call rather than cached at module load.
 *
 * WHY THIS EXISTS. `package.json` pins the dependency to the `latest`
 * dist-tag (see docs/shared-package-provenance.md), so the version a build
 * actually runs against is whatever `package-lock.json` resolved at install
 * time -- it is not a literal anywhere in this source tree. A diagnostic
 * bundle that wants to say what wire the running process is actually built
 * against (as opposed to `WIRE_PROFILE.cliVersion`, the BASELINE this file
 * pins) has to read it off disk.
 *
