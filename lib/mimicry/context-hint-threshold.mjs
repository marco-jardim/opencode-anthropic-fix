 ...
 * The `Hcp` tool-name allowlist could not be resolved from the native binary,
 * so the plugin deliberately accepts every tool name rather than inventing an
 * allowlist.
 *
 ...
 * `type` discriminator to the wire message's `role` discriminator. The native
 * `Hcp` tool-name allowlist is unresolved, so all tool names are accepted.
 *
