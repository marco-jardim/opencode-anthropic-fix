...
import {
  ...
  MAX_SUPPRESS_BETAS,
  MAX_LATCHABLE_REJECTED_BETAS,
  BODY_COUPLED_BETAS,
  UNSUPPRESSIBLE_BETAS,
  parseRejectedBetaNames,
  selectLatchableRejectedBetas,
  SESSION_ID_FALLBACK,
  PROFILE_CLI_VERSION,
  PROFILE_USER_AGENT,
} from "./adapter-input.mjs";
import { buildWireCompatibleRequest, WIRE_PROFILE } from "./wire-compat.mjs";
import { BETA_SHORTCUTS } from "../betas.mjs";
import { buildExtendedUserAgent } from "./headers.mjs";
...
describe("buildAdapterTransport — core mappings", () => {
  ...
  it("passes only the role-scoped ttl in cacheControl", () => {
    // Owner decision, revised for rc.16: `enabled` must stay ABSENT so the
    // package does not touch the plugin's tool and message cache_control
    // markers. The TTL ordering violation is resolved by pinning the identity
    // block to the SAME role-scoped ttl as the rest of the request rather than
    // by suppressing the block — suppressing it reverted the plugin's own
    // "Fix #3: Identity block has cache_control" and lost the system-prompt
    // cache hit.
    expect(transportOf().cacheControl).toEqual({ ttl: "1h" });
    expect(transportOf().cacheControl).not.toHaveProperty("enabled");
    expect(transportOf().cacheControl).not.toHaveProperty("suppressIdentityBlock");
  });

  it("downgrades the identity-block ttl to 5m on subagent turns", () => {
    // The [5m, 1h, 5m, 5m] rejection came from the identity block keeping 1h
    // while the surrounding blocks were role-scoped down to 5m.
    expect(transportOf({ isSubagent: true }).cacheControl).toEqual({ ttl: "5m" });
    expect(transportOf({ requestRole: "small" }).cacheControl).toEqual({ ttl: "5m" });
  });
...
describe("buildAdapterTransport — additionalBetas", () => {
  ...
  it("emits structured-outputs only when opted in on a supporting model", () => {
    ...
    // The exclusion list is the package's, ported from the genuine client's
    // `j4e`: claude-3-*, claude-opus-4-0 and claude-sonnet-4-0 and nothing else.
    // It replaced the plugin's own "haiku never supports it" heuristic, which
    // was UNDERINCLUSIVE — see the haiku row below.
    expect(
      transportOf({ tokenEconomy: { structured_outputs: true }, body: bodyFor({ model: "claude-opus-4-0" }) })
        .additionalBetas,
    ).not.toContain("structured-outputs-2025-12-15");
    expect(
      transportOf({
        tokenEconomy: { structured_outputs: true },
        body: bodyFor({ model: "claude-3-5-sonnet-20241022" }),
      }).additionalBetas,
    ).not.toContain("structured-outputs-2025-12-15");
  });

  it("emits structured-outputs for Haiku, which the ported predicate does not exclude", () => {
    // MIMICRY FIDELITY FIX. The plugin's local `supportsStructuredOutputs`
    // returned false for every haiku id; the genuine client's predicate excludes
    // only claude-3-*, claude-opus-4-0 and claude-sonnet-4-0. Sourcing the
    // predicate from the package seam corrected the emission for opted-in haiku
    // turns. Pinned as its own row so a silent revert is loud.
    expect(
      transportOf({ tokenEconomy: { structured_outputs: true }, body: bodyFor({ model: "claude-haiku-4-5" }) })
        .additionalBetas,
    ).toContain("structured-outputs-2025-12-15");
  });
...
describe("buildAdapterTransport — profileOverride", () => {
  ...
  it("emits a coupled userAgent + cliVersion override when the entrypoint diverges", () => {
    ...
    expect(transport.profileOverride).toEqual({
      userAgent: "claude-cli/2.1.280 (external, sdk-ts)",
  ...
  it("emits a coupled override when the agent SDK version or client app is present", () => {
    expect(transportOf({ env: { CLAUDE_AGENT_SDK_VERSION: "1.2.3" } }).profileOverride).toEqual({
      userAgent: "claude-cli/2.1.280 (external, cli, agent-sdk/1.2.3)",
      cliVersion: PROFILE_CLI_VERSION,
    });
    expect(transportOf({ env: { CLAUDE_AGENT_SDK_CLIENT_APP: "vscode" } }).profileOverride).toEqual({
      userAgent: "claude-cli/2.1.280 (external, cli, client-app/vscode)",
      cliVersion: PROFILE_CLI_VERSION,
    });
  });
...
// ---------------------------------------------------------------------------
// Step 7b — profile propagation
//
// The adapter's baseline (`PROFILE_CLI_VERSION` / `PROFILE_USER_AGENT`) is READ
// from `WIRE_PROFILE` instead of being re-typed. These tests prove the reading
// is real: the constants are referentially tied to the profile, and a SIMULATED
// profile bump moves the emitted transport with zero production edits.
// ---------------------------------------------------------------------------

describe("buildAdapterTransport — profile propagation", () => {
  it("derives its baseline from the profile rather than from re-typed literals", () => {
    expect(PROFILE_CLI_VERSION).toBe(WIRE_PROFILE.cliVersion);
    expect(PROFILE_USER_AGENT).toBe(WIRE_PROFILE.userAgent);
  });

  it("pins the CURRENT profile so a package bump surfaces as a reviewed diff", () => {
    expect(WIRE_PROFILE.cliVersion).toBe("2.1.280");
    expect(WIRE_PROFILE.userAgent).toBe("claude-cli/2.1.280 (external, cli)");
  });

  it("carries the profile version to the transport when the host detects it", () => {
    const signature = { ...makeState().signature, claudeCliVersion: WIRE_PROFILE.cliVersion };
    expect(transportOf({ signature }).profileOverride).toBeUndefined();
  });

  it("follows a simulated profile bump without touching adapter-input.mjs", async () => {
    vi.resetModules();
    vi.doMock("./wire-compat.mjs", async () => {
      const actual = await vi.importActual("./wire-compat.mjs");
      return {
        ...actual,
        WIRE_PROFILE: {
          ...actual.WIRE_PROFILE,
    ...
    try {
      const bumped = await import("./adapter-input.mjs");
      expect(bumped.PROFILE_CLI_VERSION).toBe("9.9.9");
      expect(bumped.PROFILE_USER_AGENT).toBe("claude-cli/9.9.9 (external, cli)");

      // Matching the BUMPED profile is now the silent case…
      const current = bumped.buildAdapterTransport(
        makeState({ signature: { ...makeState().signature, claudeCliVersion: "9.9.9" } }),
      );
      expect(current.applicable).toBe(true);
      expect(current.transport.profileOverride).toBeUndefined();

      // …and the previously silent version now diverges into a coupled override.
      const stale = bumped.buildAdapterTransport(
        makeState({ signature: { ...makeState().signature, claudeCliVersion: "2.1.280" } }),
      );
      expect(stale.applicable).toBe(true);
      expect(stale.transport.profileOverride).toEqual({
        userAgent: "claude-cli/2.1.280 (external, cli)",
        cliVersion: "2.1.280",
      });
    } finally {
      vi.doUnmock("./wire-compat.mjs");
      vi.resetModules();
    }
  });
});
...
// ---------------------------------------------------------------------------
// suppressBetas: session-rejected betas (custom_betas retry latch)
// ---------------------------------------------------------------------------

describe("buildAdapterTransport - suppressBetas from rejectedBetas", () => {
  const withRejected = (rejectedBetas, extra = {}) =>
    transportOf({ ...extra, signature: { ...makeState().signature, ...extra.signature, rejectedBetas } });

  it("omits suppressBetas entirely when nothing is rejected and no policy applies", () => {
    expect(transportOf()).not.toHaveProperty("suppressBetas");
    expect(withRejected([])).not.toHaveProperty("suppressBetas");
  });

  it("suppresses a rejected package-composed beta on the wire", async () => {
    const baseline = await buildWireCompatibleRequest(bodyFor(), transportOf());
    expect(baseline.headers.get("anthropic-beta").split(","), "precondition: package composes it").toContain(
      "cache-diagnosis-2026-04-07",
    );

    const transport = withRejected(["cache-diagnosis-2026-04-07"]);
    expect(transport.suppressBetas).toEqual(["cache-diagnosis-2026-04-07"]);
    const built = await buildWireCompatibleRequest(bodyFor(), transport);
    const betas = built.headers.get("anthropic-beta").split(",");
    expect(betas).not.toContain("cache-diagnosis-2026-04-07");
    expect(betas).toContain("oauth-2025-04-20");
    expect(betas).toContain("claude-code-20250219");
  });

  it("never suppresses oauth-2025-04-20 or claude-code-20250219, in any case", async () => {
    const transport = withRejected([
      "oauth-2025-04-20",
      "OAUTH-2025-04-20",
      "claude-code-20250219",
      " Claude-Code-20250219 ",
  ...
  it("skips entries outside the package beta grammar instead of failing the request", async () => {
    const transport = withRejected(["", "has space", "x".repeat(129), 42, "-leading-dash", "afk-mode-2026-01-31"]);
    expect(transport.suppressBetas).toEqual(["afk-mode-2026-01-31"]);
    await expect(buildWireCompatibleRequest(bodyFor(), transport)).resolves.toBeDefined();
  });

  it("keeps the round-robin and DISABLE_EXPERIMENTAL_BETAS suppressions unchanged, merged and deduped", () => {
    const policyOnly = transportOf({
      signature: { ...makeState().signature, strategy: "round-robin" },
      env: { CLAUDE_CODE_DISABLE_EXPERIMENTAL_BETAS: "1" },
    }).suppressBetas;
    expect(policyOnly).toContain("prompt-caching-scope-2026-01-05");

    const merged = withRejected(["cache-diagnosis-2026-04-07", "not-experimental-2020-01-01"], {
      signature: { strategy: "round-robin" },
      env: { CLAUDE_CODE_DISABLE_EXPERIMENTAL_BETAS: "1" },
    }).suppressBetas;
    // Policy entries first, in their original order; cache-diagnosis is already an
    // EXPERIMENTAL_BETA_FLAGS member so it is not duplicated.
    expect(merged.slice(0, policyOnly.length)).toEqual(policyOnly);
    expect(merged.filter((b) => b === "cache-diagnosis-2026-04-07")).toHaveLength(1);
    expect(merged).toContain("not-experimental-2020-01-01");
    expect(new Set(merged).size).toBe(merged.length);
  });

  it("caps suppressBetas at MAX_SUPPRESS_BETAS, keeping the policy suppressions", () => {
    const flood = Array.from({ length: 64 }, (_, i) => `rejected-beta-${i}`);
    const suppressed = withRejected(flood, {
      signature: { strategy: "round-robin" },
      env: { CLAUDE_CODE_DISABLE_EXPERIMENTAL_BETAS: "1" },
    }).suppressBetas;
    expect(suppressed).toHaveLength(MAX_SUPPRESS_BETAS);
    expect(suppressed).toContain("prompt-caching-scope-2026-01-05");
    expect(withRejected(flood).suppressBetas).toHaveLength(MAX_SUPPRESS_BETAS);
  });

  it("never suppresses a body-coupled beta from rejectedBetas", () => {
    const transport = withRejected([...BODY_COUPLED_BETAS, "cache-diagnosis-2026-04-07"]);
    expect(transport.suppressBetas).toEqual(["cache-diagnosis-2026-04-07"]);
  });
});

describe("BODY_COUPLED_BETAS", () => {
  it("names the registry headers whose composition is paired with a body field", () => {
    expect([...BODY_COUPLED_BETAS].sort()).toEqual(
      [
    ...
    for (const beta of UNSUPPRESSIBLE_BETAS) expect(BODY_COUPLED_BETAS.has(beta)).toBe(false);
  });

  it("thinking-display-updates is coupled: suppressing the header leaves thinking.display on the body", async () => {
    // Why the latch must never suppress it: the package writes the body half anyway.
    ...
    const transport = {
      ...buildAdapterTransport(state).transport,
      suppressBetas: ["thinking-display-updates-2026-08-18"],
    };
    const built = await buildWireCompatibleRequest(body, transport);
    expect(built.headers.get("anthropic-beta")).not.toContain("thinking-display-updates-2026-08-18");
    expect(JSON.parse(built.body).thinking.display).toBe("updates");
  });
});

describe("parseRejectedBetaNames", () => {
  const envelope = (message) => JSON.stringify({ type: "error", error: { type: "invalid_request_error", message } });

  it("extracts backtick-quoted names from the API's invalid-beta message", () => {
    expect(
      parseRejectedBetaNames(
        envelope(
          "Unexpected value(s) `cache-diagnosis-2026-04-07`, `afk-mode-2026-01-31` for the `anthropic-beta` header. Please consult our documentation at docs.claude.com or try again without the header.",
        ),
      ),
    ).toEqual(["cache-diagnosis-2026-04-07", "afk-mode-2026-01-31"]);
  });

  it("accepts single-quoted names and a raw (non-JSON) body", () => {
    expect(
      parseRejectedBetaNames("400 Unexpected value(s) 'context-hint-2026-04-09' for the 'anthropic-beta' header"),
    ).toEqual(["context-hint-2026-04-09"]);
  });

  it("returns [] for any other wording, even when a beta name appears in the body", () => {
    expect(
      parseRejectedBetaNames(envelope("Unknown beta flag cache-diagnosis-2026-04-07 in anthropic-beta header")),
    ).toEqual([]);
    expect(parseRejectedBetaNames(envelope("prompt is too long: 250000 tokens > 200000 maximum"))).toEqual([]);
    expect(parseRejectedBetaNames("")).toEqual([]);
    expect(parseRejectedBetaNames(null)).toEqual([]);
  });

  it("drops entries outside the beta grammar", () => {
    expect(
      parseRejectedBetaNames(envelope("Unexpected value(s) `ok-beta-1`, `bad beta` for the `anthropic-beta` header")),
    ).toEqual(["ok-beta-1"]);
  });

  it("accepts double-quoted and unquoted value lists", () => {
    expect(
      parseRejectedBetaNames(envelope('Unexpected value(s) "a-1", "b-2" for the "anthropic-beta" header.')),
    ).toEqual(["a-1", "b-2"]);
    expect(parseRejectedBetaNames("Unexpected value(s) a-1, b-2 for the anthropic-beta header.")).toEqual([
      "a-1",
      "b-2",
    ]);
  });

  it("fails closed when the invalid-beta sentence sits beyond the scan limit", () => {
    const late = `${"x".repeat(4096)} Unexpected value(s) \`a-1\` for the \`anthropic-beta\` header.`;
    expect(parseRejectedBetaNames(envelope(late))).toEqual([]);
  });

  it("stays linear on adversarial whitespace and repeated-prefix bodies", () => {
    const adversarial = [
      `Unexpected value(s) ${" ".repeat(16 * 1024)}x`,
      "Unexpected value(s) ".repeat(Math.ceil((16 * 1024) / 20)),
      `Unexpected value(s) ${"\t \n".repeat(5462)} for the`,
    ];
    for (const body of adversarial) {
      const start = performance.now();
      expect(parseRejectedBetaNames(body)).toEqual([]);
      expect(parseRejectedBetaNames(envelope(body))).toEqual([]);
      expect(performance.now() - start).toBeLessThan(100);
    }
  });
});

describe("selectLatchableRejectedBetas", () => {
  const msg = (...names) =>
    `Unexpected value(s) ${names.map((n) => "`" + n + "`").join(", ")} for the \`anthropic-beta\` header.`;
  const sent =
    "claude-code-20250219,oauth-2025-04-20,interleaved-thinking-2025-05-14,thinking-display-updates-2026-08-18,cache-diagnosis-2026-04-07";

  it("latches only betas that were sent AND named", () => {
    expect(selectLatchableRejectedBetas(sent, msg("cache-diagnosis-2026-04-07", "afk-mode-2026-01-31"))).toEqual({
      namedSent: ["cache-diagnosis-2026-04-07"],
      latchable: ["cache-diagnosis-2026-04-07"],
    });
  });

  it("reports but never latches UNSUPPRESSIBLE or body-coupled betas", () => {
    expect(selectLatchableRejectedBetas(sent, msg("oauth-2025-04-20", "thinking-display-updates-2026-08-18"))).toEqual({
      namedSent: ["oauth-2025-04-20", "thinking-display-updates-2026-08-18"],
      latchable: [],
    });
  });

  it("refuses an echoed header: more than MAX_LATCHABLE_REJECTED_BETAS suppressible matches", () => {
    expect(MAX_LATCHABLE_REJECTED_BETAS).toBe(2);
    expect(selectLatchableRejectedBetas(sent, msg(...sent.split(",")))).toEqual({ namedSent: [], latchable: [] });
    expect(
      selectLatchableRejectedBetas(
        sent,
        msg("oauth-2025-04-20", "interleaved-thinking-2025-05-14", "cache-diagnosis-2026-04-07"),
      ).latchable,
    ).toEqual(["interleaved-thinking-2025-05-14", "cache-diagnosis-2026-04-07"]);
  });

  it("latches nothing without the invalid-beta shape or without a sent header", () => {
    expect(selectLatchableRejectedBetas(sent, "anthropic-beta cache-diagnosis-2026-04-07 rejected")).toEqual({
      namedSent: [],
      latchable: [],
    });
    expect(selectLatchableRejectedBetas(undefined, msg("cache-diagnosis-2026-04-07")).latchable).toEqual([]);
  });
});
...
// ---------------------------------------------------------------------------
// Adapter admission (Phase 2.2)
// ---------------------------------------------------------------------------

describe("adapter pathname sets", () => {
  it("covers both spellings of both package surfaces, and nothing else", () => {
    expect([...ADAPTER_MESSAGES_PATHNAMES]).toEqual(["/v1/messages", "/messages"]);
    expect([...ADAPTER_COUNT_TOKENS_PATHNAMES]).toEqual(["/v1/messages/count_tokens", "/messages/count_tokens"]);
    // The sets are disjoint: index.mjs picks the count surface by membership in
    // the second, so an overlap would route a count turn through the main builder.
    for (const pathname of ADAPTER_COUNT_TOKENS_PATHNAMES) {
      expect(ADAPTER_MESSAGES_PATHNAMES.has(pathname)).toBe(false);
    }
  });
});

describe("assertAdapterBodyUsable", () => {
  it("accepts a JSON object body", () => {
    expect(() => assertAdapterBodyUsable(JSON.stringify({ model: "m" }), "/v1/messages")).not.toThrow();
    // Structural only: the package owns model/max_tokens/messages validation.
    expect(() => assertAdapterBodyUsable("{}", "/v1/messages")).not.toThrow();
  });

  it("rejects an absent or empty body", () => {
    expect(() => assertAdapterBodyUsable(undefined, "/v1/messages")).toThrow(/no usable body \(undefined\)/);
    expect(() => assertAdapterBodyUsable(null, "/v1/messages")).toThrow(/no usable body \(object\)/);
    expect(() => assertAdapterBodyUsable("", "/v1/messages")).toThrow(/an empty string/);
  });

  it("rejects an unparsable body and keeps the parse error as the cause", () => {
    let caught;
    try {
      assertAdapterBodyUsable("{not json", "/v1/messages");
    } catch (error) {
      caught = error;
    }

    expect(caught.message).toMatch(/could not be parsed as JSON/);
    expect(caught.cause).toBeInstanceOf(SyntaxError);
  });

  it("rejects a JSON value that is not an object", () => {
    expect(() => assertAdapterBodyUsable("[1,2]", "/v1/messages")).toThrow(/parsed as an array/);
    expect(() => assertAdapterBodyUsable("null", "/v1/messages")).toThrow(/parsed as null/);
    expect(() => assertAdapterBodyUsable("42", "/v1/messages")).toThrow(/parsed as number/);
    expect(() => assertAdapterBodyUsable(String.raw`"hi"`, "/v1/messages")).toThrow(/parsed as string/);
  });

  it("names the endpoint so a user report locates itself", () => {
    expect(() => assertAdapterBodyUsable(undefined, "/v1/messages/count_tokens")).toThrow(/v1\/messages\/count_tokens/);
    expect(() => assertAdapterBodyUsable(undefined, undefined)).toThrow(/the messages endpoint/);
  });
});
