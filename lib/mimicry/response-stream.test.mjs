 ...
 * @returns {{input: string, expectedOutput: string, expectedFallbacks: number, expectedDropped: number, knownThinkingText: string, knownTextText: string}}
 */
function buildHostCompatCase() {
  ...
  return { input, expectedOutput, expectedFallbacks, expectedDropped, knownThinkingText, knownTextText };
}
...
describe("response stream mimicry", () => {
  ...
  describe("L5: host-compat shim for opencode's pinned @ai-sdk/anthropic switch", () => {
    it("rewrites an unknown content_block_start to fallback, drops its deltas and any unknown delta in a known block, keeps content_block_stop, and leaves known blocks byte-identical", async () => {
      const { input, expectedOutput, expectedFallbacks, expectedDropped, knownThinkingText, knownTextText } =
        buildHostCompatCase();
      ...
      expect(hostCompatShimCalls).toEqual([
        [{ blockTypeFallbacks: expectedFallbacks, deltasDropped: expectedDropped }],
      ]);
    });

    it("keeps the host-compat shim's output and shim-call count partition-independent", async () => {
      const { input, expectedOutput, expectedFallbacks, expectedDropped } = buildHostCompatCase();
