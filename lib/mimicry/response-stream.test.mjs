...
describe("response stream mimicry", () => {
  ...
  describe("L5: host-compat shim for opencode's pinned @ai-sdk/anthropic switch", () => {
    ...
    it("with hostCompatShim:false, applies no shim REWRITING — unknown blocks/deltas/events pass through byte-identical", async () => {
      const { input } = buildHostCompatCase();

      const { output } = await runTransformed([new TextEncoder().encode(input)], {
    ...
    // QA fix L1: a gated-off shim (newer host SDK) must still run as a dry
    // run and report what it *would* have flagged, so a newer SDK receiving
    // types the pinned allowlist doesn't know shows up in debugLog instead
    // of staying silent — without changing a single byte of real output.
    it("with hostCompatShim:false, still reports a dry run via onHostCompatShim (dryRun:true) with no output change", async () => {
      const { input, expectedFallbacks, expectedDropped, expectedEventsDropped } = buildHostCompatCase();

      const { output, hostCompatShimCalls } = await runTransformed([new TextEncoder().encode(input)], {
        hostCompatShim: false,
      });

      // Byte-identical: the dry run never touches the real event or output.
      expect(output).toBe(input);
      expect(hostCompatShimCalls).toEqual([
        [
          {
    ...
    it("with hostCompatShim:false, never reports a dry run when nothing would have been flagged", async () => {
      const sse =
        sseEvent({ type: "message_start", message: { id: "msg_1", usage: {} } }) +
        sseEvent({ type: "content_block_start", index: 0, content_block: { type: "text" } }) +
        sseEvent({ type: "content_block_delta", index: 0, delta: { type: "text_delta", text: "all known" } }) +
        sseEvent({ type: "content_block_stop", index: 0 }) +
        sseEvent({ type: "message_stop" });
    ...
    it("with hostCompatShim:false and no onHostCompatShim callback, skips the dry run without throwing", async () => {
      const { input } = buildHostCompatCase();
      const stream = createTransformedSSEStream(makeChunkedResponse([new TextEncoder().encode(input)]), {
        onUsage: vi.fn(),
        onAccountError: vi.fn(),
        onHostCompatShim: null,
        hostCompatShim: false,
        correlationId: undefined,
        idleTimeoutMs: 0,
        captureEnabled: false,
  ...
  describe("F3: shouldApplyHostCompatShim (host-SDK version gate)", () => {
    ...
    // QA fix L1: the token regex must not match as a mere substring of a
    // different package name that happens to end in "ai-sdk".
    it("does not match a different package's UA token that merely ends in 'ai-sdk' (e.g. xai-sdk)", () => {
      // Without the token match at all, the function defaults to "apply the
      // shim" (conservative) — same as the "token absent" case above. The
      // bug this guards against is the opposite: matching `ai-sdk/anthropic/`
      // as a substring of `xai-sdk/anthropic/`, which would read version
      // 9.0.0 (newer than baseline) and wrongly report `false`.
      expect(shouldApplyHostCompatShim("xai-sdk/anthropic/9.0.0")).toBe(true);
      expect(shouldApplyHostCompatShim("some-xai-sdk/anthropic/9.0.0")).toBe(true);
    });

    it("still matches the real token after common User-Agent separators, and at string start", () => {
      // Covers both boundary forms considered for the anchor: start-of-string
      // and each of the separators real UA strings use between tokens.
      expect(shouldApplyHostCompatShim("ai-sdk/anthropic/3.0.112")).toBe(false); // start of string
      expect(shouldApplyHostCompatShim("opencode/1.18.32 ai-sdk/anthropic/3.0.112")).toBe(false); // space
      expect(shouldApplyHostCompatShim("opencode/1.18.32 (ai-sdk/anthropic/3.0.112)")).toBe(false); // (
      expect(shouldApplyHostCompatShim("opencode/1.18.32;ai-sdk/anthropic/3.0.112")).toBe(false); // ;
      expect(shouldApplyHostCompatShim("opencode/1.18.32,ai-sdk/anthropic/3.0.112")).toBe(false); // ,
    });
  });
  ...
  // ---------------------------------------------------------------------------
  // QA fix L4: cancelling the transformed stream must propagate cancellation
  // to the upstream source reader, instead of leaving the real connection
  // dangling once the consumer walks away. Both the SSE path and the F5
  // passthrough path return their own ReadableStream, so both need it.
  // ---------------------------------------------------------------------------
  describe("L4: stream cancellation propagates to the source reader", () => {
    /**
     * Build a Response whose body never closes on its own, so any observed
     * termination must have come from an explicit cancel — and whose
     * underlying source records the cancel reason it's given.
     * @param {{contentType?: string | null}} [options]
     * @returns {{response: Response, cancelSpy: ReturnType<typeof vi.fn>}}
     */
    function makeNeverClosingResponse({ contentType = "text/event-stream" } = {}) {
      ...
      const response = new Response(
        new ReadableStream({
          start(controller) {
            ...
            // Deliberately never closes — a real stalled/long-lived connection.
          },
          cancel(reason) {
            cancelSpy(reason);
          },
    ...
    it("SSE path: cancelling the transformed stream cancels the upstream reader with the same reason", async () => {
      const { response, cancelSpy } = makeNeverClosingResponse();
      const stream = createTransformedSSEStream(response, {
        onUsage: vi.fn(),
        onAccountError: vi.fn(),
        onHostCompatShim: vi.fn(),
      ...
      await stream.cancel("consumer done");

      expect(cancelSpy).toHaveBeenCalledWith("consumer done");
    });

    it("F5 passthrough path (non-SSE): cancelling the transformed stream cancels the upstream reader with the same reason", async () => {
      const { response, cancelSpy } = makeNeverClosingResponse({ contentType: "application/json" });
      const stream = createTransformedSSEStream(response, {
        onUsage: vi.fn(),
        onAccountError: vi.fn(),
        onHostCompatShim: vi.fn(),
    ...
    it("SSE path: cancelling via an acquired reader still cancels the upstream reader", async () => {
      const { response, cancelSpy } = makeNeverClosingResponse();
      const stream = createTransformedSSEStream(response, {
        onUsage: vi.fn(),
        onAccountError: vi.fn(),
      ...
      const reader = stream.getReader();
      await reader.read(); // consume the first (only) chunk

      await reader.cancel("no longer needed");

      expect(cancelSpy).toHaveBeenCalledWith("no longer needed");
    });
  });
});
