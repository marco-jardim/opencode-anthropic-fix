 ...
 * @returns {string}
 */
export function buildAnthropicBillingHeader(version, firstUserMessage, provider, workloadOverride) {
...
export function buildSystemPromptBlocks(system, signature) {
  ...
  //
  // This is CONDITIONAL, never unconditional: the legacy path (non-Anthropic
  // provider, or signature emulation off) still builds the request through
  // buildRequestHeaders and depends on these two blocks being present. Dropping
  // them for every caller would break Bedrock/Vertex silently — no error, just
  // a wrong request on the wire.
  const emittedBlocks =
