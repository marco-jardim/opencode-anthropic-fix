 ...
 */
const HOST_SDK_USER_AGENT_RE = /ai-sdk\/anthropic\/(\d+)\.(\d+)\.(\d+)/;
 ...
 * and a Content-Length copied from the original response by the caller
 * (before this stream is consumed) stays correct. This intentionally skips
 * everything the SSE pipeline does: usage extraction, account-error
 * detection, mcp_ tool-name rewriting, the L5 host-compat shim, and the SSE
 * debug capture side-channel are all SSE-specific and never apply to a
 * non-SSE body. Only the idle-timeout watchdog is still honored.
 * @param {ReadableStreamDefaultReader<Uint8Array>} reader
 * @param {number} idleTimeoutMs
 ...
 * contract, strict or lenient, and a Content-Length copied from the
 * original response by the caller stays correct.
 *
 ...
 * @param {((stats: {blockTypeFallbacks: number, deltasDropped: number, eventsDropped: number}) => void) | null} [options.onHostCompatShim]
 ...
 *   "only fire if something happened" shape. Never called when
 *   `hostCompatShim` is `false`, or for a non-SSE body.
 ...
 *   flag. `false` means no shim rewriting at all: unknown block/delta/event
 *   types and unknown citation types all pass through byte-identical, and
 *   `onHostCompatShim` never fires.
...
export function createTransformedSSEStream(
  ...
  function finalizeBlock() {
    ...
    const shimAction = hostCompatShim ? applyHostCompatShim(parsed, hostCompatShimState) : "keep";
