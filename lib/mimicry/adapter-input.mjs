 ...
 *
 * `rejectedBetas` carries the canonical betas the API rejected this session
 * (the `customBetasStripped` one-shot and the TTL'd `sessionRejectedBetas`
 * latch in index.mjs). Dropping them from `customBetas` alone is not enough
 * once the package composes the same beta itself (e.g.
 * `cache-diagnosis-2026-04-07` under 2.1.280), so they are suppressed here too.
 * `UNSUPPRESSIBLE_BETAS` are never taken from this input, entries outside the
 * package grammar are skipped (they cannot be on the wire anyway), and the list
 * is capped at `MAX_SUPPRESS_BETAS` with the policy suppressions taking
 * precedence.
 *
...
function buildSuppressBetas({ isRoundRobin, disableExperimentalBetas, rejectedBetas }) {
  ...
  for (const rejected of rejectedBetas ?? []) {
    ...
    if (UNSUPPRESSIBLE_BETAS.has(beta.toLowerCase())) continue;
    suppressed.add(beta);
  }
 ...
 *   `rejectedBetas`: canonical betas the API rejected this session; routed to `suppressBetas`.
