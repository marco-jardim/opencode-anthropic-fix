...
export class AccountManager {
  ...
  #seedEnvAccount(env) {
    ...
    const now = Date.now();
    this.#accounts.push({
      id: `env:${now}:${hashTokenFragment(headless.refreshToken)}`,
      index: this.#accounts.length,
      email: undefined,
  ...
  async syncActiveIndexFromDisk() {
    ...
    if (diskSnapshot !== memSnapshot) {
      ...
      this.#accounts = [...rebuilt, ...envAccounts];
      this.#accounts.forEach((acc, i) => {
        acc.index = i;
      });
      ...
      // A CLI logout-all can empty the disk list while an env account survives.
      if (this.#currentIndex >= this.#accounts.length) {
        this.#currentIndex = this.#accounts.length - 1;
