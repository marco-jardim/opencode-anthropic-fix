...
//
// Every endpoint, user-agent and beta name used here is imported from
// ./oauth-constants.mjs, which transcribes docs/oauth-2.1.280-contract.md.
// This module owns OAuth *behaviour*; it owns no OAuth *values*.
// ---------------------------------------------------------------------------
 ...
 *
 * There is exactly one fingerprint. Claude Code 2.1.280 has no axios path, so
 * a switch selecting one would not make a user more anonymous — it would make
 * them the only client on the network sending that shape.
 *
 * Header **order** is part of the fingerprint: the object literal is built in
 * emission order and must never be sorted or spread through a helper that
 * reorders it.
 * @returns {Record<string, string>}
 * @see docs/oauth-2.1.280-contract.md §2.3, §2.5
 */
 ...
 * returns `false` immediately. With the gate on the RFC 7009 request is sent,
 * carrying the same SDK fingerprint as the other token requests rather than the
 * retired axios one — if a user opts into being identifiable, they should at
 * least not be identifiable twice over.
 *
 * Callers must proceed with local cleanup regardless of the result.
 *
...
export async function revoke(refreshToken, options = {}) {
  if (!revokeOnLogoutEnabled(options.revokeOnLogout)) return false;
  try {
 ...
 * ignored — it is retained so existing call sites keep working.
 *
 ...
 *   `scopes` and `sdkTokenUserAgent` are accepted and ignored.
