...
export async function authorize(mode, options = {}) {
  ...
  // the attested behaviour and is enforced here.
  //
  // NOT ATTESTED: what an APPROVED value is then used for. §13.8 records the
  // validation, not the consumption. Redirecting the flow to the approved value
  // would be an invention, so this plugin validates and does not redirect — the
  // guard runs, the endpoint constants stand. Recorded as divergence D14.
  resolveCustomOAuthUrl(options?.env ?? process.env);
