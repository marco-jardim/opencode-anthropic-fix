 ...
 * conditional extensions. This binding is retained as a re-export so existing
 * importers of `CLAUDE_AI_SCOPES` keep resolving.
 *
 ...
 * state in a test without touching the user's config file. Config is read live
 * (never cached in a closed-over constant) because `/anthropic set` mutates it
 * at runtime. A config read that throws resolves to the defaults rather than
 * failing the login.
 * @param {{pluginsRegistered?: boolean, projectScopes?: string[]}} options
 * @returns {{pluginsRegistered: boolean, projectScopes: string[]}}
...
function resolveScopeOptions(options) {
  ...
  return {
    pluginsRegistered:
      typeof options.pluginsRegistered === "boolean" ? options.pluginsRegistered : oauth?.plugins_scope === true,
    projectScopes: Array.isArray(options.projectScopes)
...
export async function authorize(mode, options = {}) {
  ...
  const { pluginsRegistered, projectScopes } = resolveScopeOptions(options);
  ...
  url.searchParams.set("scope", serializeScopes(composeScopes({ mode, pluginsRegistered, projectScopes })));
