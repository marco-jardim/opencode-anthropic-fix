...
import { CLIENT_ID, loadConfig } from "./config.mjs";
...
// Claude Code 2.1.195 migrated the OAuth token client from axios to the
// Anthropic TS SDK's native-fetch `userOAuthProvider`. When the
// `oauth.sdk_token_useragent` config flag is ON, the token POST switches to
// this fingerprint: `User-Agent: anthropic-sdk-typescript/0.94.0
// userOAuthProvider`, `anthropic-beta: oauth-2025-04-20`, JSON body, and NO
// axios `Accept` header (native fetch sends no default Accept). The SDK version
// is the one real Claude Code 2.1.195 shipped ("0.94.0"); it is hardcoded here
// so the OAuth token path stays decoupled from the wire seam
// (lib/mimicry/wire-compat.mjs), which only describes /v1/messages traffic.
const OAUTH_SDK_VERSION = "0.94.0";
const OAUTH_SDK_USER_AGENT = `anthropic-sdk-typescript/${OAUTH_SDK_VERSION} userOAuthProvider`;
const OAUTH_SDK_BETA = "oauth-2025-04-20";
...
/**
 * Resolve whether the SDK-native-fetch token fingerprint should be used.
 * An explicit boolean override (used by callers/tests) wins; otherwise the
 * live `oauth.sdk_token_useragent` config flag is read (default true).
 ...
 * @param {boolean} useSdk - When true, emit the CC 2.1.195 SDK-native-fetch
 *   fingerprint; when false, emit the legacy axios fingerprint (default).
 * @returns {Record<string, string>}
 */
function buildTokenHeaders(useSdk) {
  if (useSdk) {
    // Native fetch sends no default Accept header — omit it deliberately.
    return {
 ...
 * @param {{ sdkTokenUserAgent?: boolean }} [options] - `sdkTokenUserAgent`
 *   overrides the `oauth.sdk_token_useragent` config flag (mainly for tests).
 * @returns {Promise<
 ...
 * >}
 */
export async function exchange(code, verifier, options = {}) {
 ...
 *   `sdkTokenUserAgent` overrides the `oauth.sdk_token_useragent` config flag
 *   (mainly for tests).
