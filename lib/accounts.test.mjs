...
describe("AccountManager.load", () => {
  ...
  it("loads stored accounts from disk", async () => {
    loadAccounts.mockResolvedValue(
      makeAccountsData([{ lastUsed: 2000 }, { lastUsed: 4000 }], { activeIndex: 1 }),
    );
    const manager = await AccountManager.load(DEFAULT_CONFIG, null);
...
describe("AccountManager merge-on-save", () => {
  ...
  it("clears deltas after save", async () => {
    ...
    loadAccounts.mockResolvedValue(makeAccountsData([{
      id: accountId,
      refreshToken: "tok-1",
      stats: { requests: 10, inputTokens: 1000, outputTokens: 500, cacheReadTokens: 0, cacheWriteTokens: 0, lastReset: 1000 },
    }]));
    saveAccounts.mockResolvedValue(undefined);
    ...
    loadAccounts.mockResolvedValue(makeAccountsData([{
      id: accountId,
      refreshToken: "tok-1",
      stats: { requests: 11, inputTokens: 1100, outputTokens: 500, cacheReadTokens: 0, cacheWriteTokens: 0, lastReset: 1000 },
    }]));
