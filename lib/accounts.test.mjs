...
import { AccountManager, RATE_LIMIT_KEY_FAST } from "./accounts.mjs";
...
describe("AccountManager headless env account (T-3)", () => {
  ...
  it.each([
    ["bootstrap", null],
    ["stored accounts", makeAccountsData([{}])],
  ])("T-3.2 rejects incomplete scopes on the %s path", async (_path, stored) => {
    loadAccounts.mockResolvedValue(stored);
    await expect(
      AccountManager.load(DEFAULT_CONFIG, null, envWith({ CLAUDE_CODE_OAUTH_SCOPES: "user:profile" })),
    ).rejects.toThrow(/Missing: user:inference/);
  });
  ...
  it("T-3.4c writes an empty array and safe activeIndex for an env-only pool", async () => {
    loadAccounts.mockResolvedValue(null);
    const manager = await AccountManager.load(DEFAULT_CONFIG, null, envWith());
    await manager.saveToDisk();
