...
function buildBetaSyncSection(diff) {
  if (!diff || !diff.fields) return "";

  const betaFields = ["allBetaFlags", "alwaysOnBetas", "experimentalBetas", "bedrockUnsupported"];
  const changeLines = [];

  for (const key of betaFields) {
    if (!(key in diff.fields)) continue;

    const { from = [], to = [] } = diff.fields[key];
    const added = to.filter((flag) => !from.includes(flag));
    const removed = from.filter((flag) => !to.includes(flag));
    if (added.length === 0 && removed.length === 0) continue;

    const changes = [];
    if (added.length > 0) changes.push(`added: ${added.map((flag) => `\`${flag}\``).join(", ")}`);
    if (removed.length > 0) changes.push(`removed: ${removed.map((flag) => `\`${flag}\``).join(", ")}`);
    changeLines.push(`- \`${key}\` ${changes.join("; ")}`);
  }
...
Beta flags changed — update \`docs/mimicry/beta-decision-table.md\` accordingly:

${changeLines.join("\n")}

When the wire behavior changes, add a redacted replay fixture (see \`test/fixtures/requests/\`):

\`\`\`json
{
  "meta": { "note": "fill + redact before committing" },
  "request": {
    "method": "POST",
    "url": "/v1/messages",
    "headers": {},
...
function buildPRBody(version, baseline, extracted, diff) {
  ...
  const betaSync = buildBetaSyncSection(diff);
