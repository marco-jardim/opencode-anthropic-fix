...
const deliberateOmissions = new Map([
  ...
  [
    ...
    // and still present in 2.1.280 — the library's 2.1.280 behaviour-flag audit
    // confirms the bundle's billing-block builder still emits both `cc_prev_req`
    // and `cc_prompt_id` with the same validation patterns this package declares.
    ...
    // genuine 2.1.233+ / 2.1.280 client emits `cc_prev_req=req_...;` and the
    // plugin does not. Recorded in docs/mimicry/wire-compat-divergences.md;
    // closing it needs response-id capture plumbed through the interceptor,
    // which is a follow-up.
    "the plugin does not track server-returned request ids, so it cannot supply `cc_prev_req`; known turn-2+ divergence from the 2.1.233+ / 2.1.280 profile, tracked as a follow-up",
  ],
  [
    "promptId",
    // Feeds the `cc_prompt_id` segment of the same 2.1.233+ / 2.1.280 billing
    // block. Upstream reads a host-side prompt UUID; the package cannot derive
    // one and neither can the plugin — nothing in the intercepted request
    // identifies a prompt, and inventing a UUID would emit a segment whose
    // value is meaningless rather than matching genuine traffic.
    //
    // Same class of divergence as `previousRequestId`, same follow-up.
    "no host-side prompt UUID reaches the interceptor; synthesising one would emit a fabricated `cc_prompt_id`, so the segment is omitted — known 2.1.233+ / 2.1.280 divergence, tracked as a follow-up",
  ],
]);
