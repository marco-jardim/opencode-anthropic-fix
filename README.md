...
- **Billing header fingerprint parity** &mdash; `cc_version` suffix uses the real CLI's 3-char fingerprint hash (SHA-256 of salt + first user message chars + version), `cch` matches the Bun native client attestation placeholder (omitted on bedrock/anthropicAws/mantle), and `X-Claude-Code-Session-Id` header is sent on all requests
